...
out = open("train_refine_gan2.log", 'w')
...
RESULT_SAVE_DIR = 'refine_results_gan2/'
...
for num_epoch in range(nb_epoch):
    for iters in range(samples_per_epoch / batch_size):
        ...
        _, summary_str, err_d = sess.run([d_optim, d_sum, refinenet.d_loss], feed_dict={refinenet.x1: batch_x1,  refinenet.x2: batch_x2, refinenet.y: batch_y, lr: learning_rate})
        writer.add_summary(summary_str, counter)
        
        _, summary_str, err_g, err_img = sess.run([g_optim, g_sum, refinenet.g_loss, refinenet.img_loss], feed_dict={refinenet.x1: batch_x1,  refinenet.x2: batch_x2, refinenet.y: batch_y, lr: learning_rate})
        writer.add_summary(summary_str, counter)

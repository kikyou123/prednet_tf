...
RESULT_SAVE_DIR = 'refine_results/'

checkpoint_dir = os.path.join(RESULT_SAVE_DIR, 'models/')
saves_dir = os.path.join(RESULT_SAVE_DIR, 'evals_nogan/')
...
for iters in range(X1_test.shape[0]):
    ...
    samples, diff_imgs = sess.run([refinenet.out, refinenet.diff], {refinenet.x1: x1_test, refinenet.x2: x2_test, refinenet.y: y_test})
    x2_test = inverse_transform(x2_test)
    y_test = inverse_transform(y_test)
    samples = inverse_transform(samples)
    diff_imgs = inverse_transform(diff_imgs)
    gen_samples = np.concatenate((y_test, x2_test, samples, diff_imgs), axis=0)
    save_images(gen_samples, [4, batch_size], saves_dir + 'test_%s.jpg' % (iters))
